...
class Env:
    ...
    def add(self, n_tigers, n_deers):
        ...
        self.deer_group.number_of_members = self.n_deers
    ...
    def transition(self):
        ...
        for deer_index, deer in enumerate(self.deer_group):
            ...
            # print(f"Deer at{deer.pos}")

            if not self.is_simulating and not self.deer_not_learning:
                self.deer_Qs.update(deer.Q)
            if deer.got_caught:  # and not self.is_training:
                if self.is_training:
                    self.deer_group.number_of_members -= 1
                else:
                    deer.kill()
                # print(f"Deer got killed")
    ...
    def reward_of(self):
        ...
        for tiger in self.tiger_group:
            ...
            if len(self.tiger_group) > 1:
                ...
                if tiger.prev_closest_deer_distance is not None:
                    tiger.reward += round((tiger.prev_closest_deer_distance - closest_deer_distance) / 100.0, 2)
                tiger.prev_closest_deer_distance = closest_deer_distance
        ...
        for deer in self.deer_group:
            ...
            if deer.prev_closest_tiger_distance is not None:
                deer.reward -= round((deer.prev_closest_tiger_distance - closest_tiger_distance) / 100.0, 2)

            if deer.previous_position is not None:
                deer.reward += 0.1 * (deer.pos == deer.previous_position)

            deer.previous_position = deer.pos
    ...
    def tiger_wins(self):
        if self.is_training:
            return self.deer_group.number_of_members <= 0
